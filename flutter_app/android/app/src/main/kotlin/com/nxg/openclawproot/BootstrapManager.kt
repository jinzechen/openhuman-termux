...
class BootstrapManager(
    ...
    fun setupDirectories() {
        listOf(rootfsDir, tmpDir, homeDir, configDir, "$homeDir/.openhuman", libDir).forEach {
            File(it).mkdirs()
        }
    ...
    fun isBootstrapComplete(): Boolean {
        ...
        val bypass = File("$rootfsDir/root/.openhuman/bionic-bypass.js")
        val node = File("$rootfsDir/usr/local/bin/node")
        val openhuman = File("$rootfsDir/usr/local/lib/node_modules/openhuman/package.json")
        return rootfs.exists() && binBash.exists() && bypass.exists()
            && node.exists() && openhuman.exists()
    }
    ...
    fun getBootstrapStatus(): Map<String, Any> {
        ...
        val openhumanExists = File("$rootfsDir/usr/local/lib/node_modules/openhuman/package.json").exists()
        val bypassExists = File("$rootfsDir/root/.openhuman/bionic-bypass.js").exists()

        return mapOf(
            "rootfsExists" to rootfsExists,
            "binBashExists" to binBashExists,
            "nodeInstalled" to nodeExists,
            "openhumanInstalled" to openhumanExists,
            "bypassInstalled" to bypassExists,
            "rootfsPath" to rootfsDir,
            "complete" to (rootfsExists && binBashExists && bypassExists
                && nodeExists && openhumanExists)
        )
    }
    ...
    private fun configureRootfs() {
        ...
        aptConfDir.mkdirs()
        File(aptConfDir, "01-openhuman-proot").writeText(
        ...
        dpkgConfDir.mkdirs()
        File(dpkgConfDir, "01-openhuman-proot").writeText(
            "force-unsafe-io\n" +
            "no-debsig\n" +
            "force-overwrite\n" +
            "force-depends\n" +
        ...
        listOf(
            ...
            // OpenHuman runtime directories (can't mkdir at runtime)
            "$rootfsDir/root/.openhuman",
            "$rootfsDir/root/.openhuman/data",
            "$rootfsDir/root/.openhuman/memory",
            "$rootfsDir/root/.openhuman/skills",
            "$rootfsDir/root/.openhuman/config",
            "$rootfsDir/root/.openhuman/extensions",
            "$rootfsDir/root/.openhuman/logs",
            "$rootfsDir/root/.config/openhuman",
            "$rootfsDir/root/.local/share",
            "$rootfsDir/root/.cache",
            "$rootfsDir/root/.cache/openhuman",
    ...
    fun createBinWrappers(packageName: String) {
        ...
        // "bin": "cli.js"  OR  "bin": {"openhuman": "bin/openhuman.js", ...}
    ...
    fun installBionicBypass() {
        val bypassDir = File("$rootfsDir/root/.openhuman")
        if (!bypassDir.exists()) {
            bypassDir.mkdirs()
        }
        ...
        val cwdFixContent = """
// OpenHuman CWD Fix - Auto-generated
...
""".trimIndent()
        ...
        //    Usage: node /root/.openhuman/node-wrapper.js <script> [args...]
        val wrapperContent = """
// OpenHuman Node Wrapper - Auto-generated
...
require('/root/.openhuman/proot-compat.js');
...
// OpenHuman Proot Compatibility Layer - Auto-generated
...
""".trimIndent()
        ...
        // 4. Bionic bypass — comprehensive runtime patcher for openhuman.
        //    Loaded via NODE_OPTIONS="--require /root/.openhuman/bionic-bypass.js"
        val bypassContent = """
// OpenHuman Bionic Bypass - Auto-generated
...
""".trimIndent()
        ...
        val exportLine = "export NODE_OPTIONS=\"--require /root/.openhuman/bionic-bypass.js\""

        val existing = if (bashrc.exists()) bashrc.readText() else ""
        if (!existing.contains("bionic-bypass")) {
            bashrc.appendText("\n# OpenHuman Bionic Bypass\n$exportLine\n")
        }

        // Pre-seed openhuman.json with gateway.mode=local so the gateway
        // doesn't reject startup with "set gateway.mode=local" (#93, #90).
        val configFile = File(bypassDir, "openhuman.json")
...
""".trimIndent())
    ...
    /** Read a file from inside the rootfs (e.g. /root/.openhuman/openhuman.json). */
    ...
    private fun checkOpenHumanInProot(): Boolean {
        return try {
            val pm = ProcessManager(filesDir, nativeLibDir)
            val output = pm.runInProotSync("command -v openhuman")
